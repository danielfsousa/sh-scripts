...
if [[ $current_branch == "master" ]]; then
  echo "You're already on master, create a new branch, push it, and then run this script"
  exit 1
fi
...
if [[ status_code == "201" ]]; then
  ...
  echo "Error ocurred, $status_code status received" >&2
  exit 1
fi
